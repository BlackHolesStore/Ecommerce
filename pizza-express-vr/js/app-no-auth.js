  ...
  if (gameContainer) {
      ...
      ">
        ...
        <button 
          ...
          style="
            ...
            letter-spacing: 2px;
          "
          onmouseover="this.style.transform='scale(1.05)'; this.style.boxShadow='0 12px 20px rgba(102, 126, 234, 0.5)'; this.style.borderColor='#667eea';"
          onmouseout="this.style.transform='scale(1)'; this.style.boxShadow='0 8px 15px rgba(0, 0, 0, 0.4)'; this.style.borderColor='white';"
        >
          🥽 ENTER VR
        </button>
    ...
    if (playBtn) {
      playBtn.addEventListener('click', () => {
        ...
        // Game URL - redirect to /game route
        const gameUrl = '/game';
    ...
    // Add click handler to VR button - launches directly into VR experience
      ...
      vrBtn.addEventListener('click', async () => {
        ...
        if (navigator.xr) {
          try {
            ...
            if (vrSupported) {
              console.log("✅ WebXR VR supported - launching game in VR mode");
              // Go to game with VR auto-start parameter
              window.location.href = '/game?vr=true';
            } else {
              alert('🥽 VR not supported on this device.\n\nPlease use a VR headset like Meta Quest to enter VR mode.');
            }
          } catch (err) {
            console.error("WebXR check failed:", err);
            alert('🥽 Could not check VR support.\n\nTry opening this page on your Meta Quest headset.');
          }
        } else {
          alert('🥽 WebXR not available.\n\nPlease open this page in the Meta Quest Browser to enter VR.');
        }
      });
    }
  }
